...
import cl.novandi.barajas.util.EmailUtil;
import cl.novandi.barajas.util.Mail;
...
public class LoginController {
	...
	public ResponseEntity<Integer> enviarCorreo(@RequestBody String correo) {
		...
		try {
			Usuario us = service.verificarEmail(correo);
			if (us != null && us.getId() > 0) {
				...
				mail.setSubject("RESTABLECER CONTRASEÑA - BARAJASAPP");
				
				Map<String, Object> model = new HashMap<>();
				String url = "http://localhost:4200/recuperar/" + token.getToken();
				model.put("user", token.getUsuario().getEmail());
	...
	public ResponseEntity<Integer> restablecerClave(@PathVariable("token") String token, @RequestBody String clave) {
		...
		try {
			...
			rpta = service.cambiarPassword(claveHash, rt.getUsuario().getEmail());
